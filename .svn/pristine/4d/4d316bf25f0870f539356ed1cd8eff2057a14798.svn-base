...
public class StepDetector implements SensorEventListener {
	...
	public StepDetector(Context context) {
		...
		sharedPreferences = SettingsActivity.sharedPreferences;
		SENSITIVITY = sharedPreferences.getInt(
				SettingsActivity.SENSITIVITY_VALUE,
				SettingsActivity.Default_Sersitivity_Value);
		editor = sharedPreferences.edit();
		CURRENT_SETP = sharedPreferences.getInt("CURRENT_SETP", 0);
		mCalendar = Calendar.getInstance();// 获取当天Calendar对象
							...
							if (isAlmostAsLargeAsPrevious
								...
								if (end - start > 500) {// 此时判断为走了一步
									Log.i("StepDetector", "CURRENT_SETP:"
											+ CURRENT_SETP);
									CURRENT_SETP++;
									// 判断下是否又是新的一天了，是，则步数清零
									if (mCalendar.get(Calendar.DAY_OF_WEEK) != sharedPreferences
											.getInt("WEEK", -1)) {
										CURRENT_SETP = 0;
									}
									editor.putInt("CURRENT_SETP", CURRENT_SETP);
									editor.putInt("WEEK",
